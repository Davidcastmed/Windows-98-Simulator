...
export class FileExplorerComponent {
  ...
  currentPath = signal<string[]>(['C:', 'My Documents']);
  
  currentDirectoryContent = computed(() => {
    let current: Directory | undefined = this.fileSystem;
    for (let i = 1; i < this.currentPath().length; i++) {
      const segment = this.currentPath()[i];
      current = current.children.find(node => node.type === 'directory' && node.name === segment) as Directory;
      if (!current) {
